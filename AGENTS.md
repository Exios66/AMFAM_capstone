...
- **Manifests** — JSONL checkpoint files in `reports/manifests/` enable resumable eval runs after interruption. `ManifestStore` (`src/evaluation.py`) tracks per-row status; `--manifest` flag on `braintrust_openrouter_input.py`. Each completed record carries a `tag` (`OK`/`MISS!`/`ERROR!`), `runner_up`, `cost`, **and the full `reasoning` trace + `raw_response`** (the eval runner writes them to the manifest, never only to Braintrust spans, so reduced-spec/`--agent` runs whose spans live on AMFAMv4 are still fully scorable locally); `score_manifest.load_manifest()` derives the tag in-memory when absent. For manifests written before reasoning-in-manifest shipped, `score_manifest.py --persist-manifest` backfills reasoning/raw_response from Braintrust spans (primary project, then the agent account) and rewrites the file in place.

**Eval runner** (`braintrust_openrouter_input.py`): wraps an OpenAI client pointed at OpenRouter with `braintrust.wrap_openai()`. Uses `braintrust.Eval()` to run the classification task with `max_concurrency=8`. Retries transient provider failures (502s, token caps, empty responses) up to `MAX_TRIES=3`; grows `max_tokens` on `finish_reason=length` up to `MAX_TOKENS_CAP=32768`. Every manifest record (completed or errored) carries the `reasoning` trace + `raw_response` — the manifest, not the Braintrust span log, is the durable record.

**Scoring tools** (`score_manifest.py`): derives the full metric set from a manifest alone (`--no-backfill`), or backfills reasoning/raw_response/runner_up/cost from Braintrust spans across the primary project **and the agent account** (`--agent` runs log to AMFAMv4), with `--persist-manifest` to rewrite the backfilled reasoning into the manifest itself. The report/visual chain (`summarize_braintrust_experiment.py`, `braintrust_report.py`, `braintrust_report_manifest.py`, `braintrust_metrics_visual.py`) accepts `--agent` to read agent-org experiments; `braintrust_metrics_visual.py` merges resume-loop version-suffixed experiments and dedupes by filename.
