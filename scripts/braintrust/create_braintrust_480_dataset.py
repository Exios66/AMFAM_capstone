"""
Build a balanced 480-image RVL-CDIP dataset that CONTAINS the original 160-image
``fixed_size_sampled`` dataset, then upload it to Braintrust as
``fixed_size_sampled_480`` (30 images per class x 16 classes).

Composition:
    * The exact 160 rows of the existing ``fixed_size_sampled`` dataset are loaded
      from Braintrust (attachment bytes are downloaded; nothing is regenerated),
      so those images are byte-identical to the current production set.
    * Fresh RVL-CDIP test images (from the public Hugging Face parquet mirror
      ``jordyvl/rvl_cdip_100_examples_per_class``) are sampled deterministically
      (seed 42) to bring every class up to 30 images. Fresh images are converted
      to 1024x1024 grayscale PNGs (aspect-ratio preserving, white padding, 300 DPI
      metadata) and are de-duplicated against the original 160 by raw pixel hash.
    * Per-class counts: 14 classes start at 10/160, ``news_article`` at 9 and
      ``scientific_publication`` at 11, so the top-up targets 21/19 respectively
      to land on exactly 30 per class (480 total).
...
Usage:
    ...
    python scripts/braintrust/create_braintrust_480_dataset.py --output-dir fixed_size_sampled_480
"""
...
DEFAULT_DATASET = "fixed_size_sampled_480"
...
def sample_fresh_rows(
    ...
    """Sample fresh images so each class reaches TARGET_PER_CLASS.

    ``originals`` already have ``png_bytes`` set. Fresh candidates that are
    pixel-identical to an original (or to an earlier accepted fresh image) are
    skipped. Returns records shaped like ``{label, png_bytes, filename,
    ...
    for class_name in sorted(by_class):
        needed = TARGET_PER_CLASS - existing.get(class_name, 0)
    ...
    for class_name in sorted(by_class):
        print(f"  {class_name:<24} {existing.get(class_name, 0)} + {sum(1 for r in fresh if r['label'] == class_name)} = {TARGET_PER_CLASS}")
    return fresh
...
def upload_dataset(
    ...
    """Insert original + fresh rows into a Braintrust dataset."""
    ...
    experiment = braintrust.init_experiment(
        ...
        description=f"Build {len(originals) + len(fresh)}-image RVL-CDIP slice (160 original + {len(fresh)} fresh) "
        ...
        metadata={
            ...
            "seed": seed,
            "images": len(originals) + len(fresh),
        },
    )

    def to_record(record: dict, subset: str, source: str, doc_id: str) -> dict:
        return {
            "label": record["label"],
    ...
    records = [
        to_record(r, "original_160", "braintrust_fixed_size_sampled", f"original_{r['filename']}")
        for r in originals
    ]
    records += [
        to_record(r, "fresh_320", "rvl_cdip_hf_parquet", f"rvl_cdip_{r['source_index']}")
        for r in fresh
    ]
    ...
    if output_dir is not None:
        ...
        print(f"Wrote {len(records)} PNGs to {output_dir}")
...
def main() -> None:
    ...
    parser.add_argument("--source-dataset", default=SOURCE_DATASET,
                        help=f"Existing dataset holding the original 160 (default: {SOURCE_DATASET})")
    ...
    parser.add_argument("--output-dir", type=Path, default=None,
                        help="Optional: also write all 480 PNGs here")
    ...
    try:
        ...
        fresh = sample_fresh_rows(rows, originals, args.seed, target_size)
    finally:
