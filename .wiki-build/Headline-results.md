...
![Accuracy progress across prompt versions and dataset slices](assets/charts/accuracy_progress.svg)
...
The full per-run record lives in the [experiment log](Experiment-Log) and the
...
![Per-class accuracy on the 1,120-image slice](assets/charts/per_class_accuracy_qwen3.7-flash_v11.8_reasoning_1600_balanced_1120.svg)
...
[misclassification appendix](Misclassifications) — each trace
...
See [cost analysis](Cost-Estimation) for per-model pricing.
...
1. **Fix the 72 near-misses** — sharpen tie-break rules between the top confused
   pairs (highest leverage, up to ~6.4 *pp*).
