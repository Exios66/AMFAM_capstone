...
| `prompts.py` | Versioned classification prompts `PROMPT_V1`…`PROMPT_V11`, the `PROMPTS` map, `get_prompt()`, `list_prompt_versions()`, `DEFAULT_PROMPT_VERSION` (`v4`). |
...
`prompts.py` carries the full evolution of the classification prompt (v1 baseline through v11).
Versions differ in disambiguation rules, decision-cascade structure, and the optional scratchpad
