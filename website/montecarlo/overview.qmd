...
subtitle: 'Zero-spend what-if analysis over the 4,641-row / 1,512-image Braintrust reasoning corpus.'
toc: true
toc-depth: 3
---

::: {.callout-note title="Zero-Spend Methodology"}
...
- **Abstention/escalation**: route the lowest-confidence `α` fraction to a
...
::: {.callout-important title="Top Findings"}
- **Ensemble voting is a weak lever.** Simulated committee accuracy is 82.1% at
  *K* = 1 → 85.3% at *K* = 10 → 86.3% at *K* = 25. Cross-run variance is small; majority
  voting buys at most ~4 *pp* at 10–25× cost. The variance budget lives in prompt
...
- **Confidence-gated escalation is the accuracy lever.** Routing the lowest
  ...
  40% escalation reaches ~92% at 1.8× cost. The 40→50% step is non-monotonic
...
- **The fallback-model salvage is the failure lever.** With fallback on, the
  simulated failure rate drops from ~2.9% to ~0.11% — a **~25× reduction**. At
