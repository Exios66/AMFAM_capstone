...
**The mechanism.** These words appear in documents that *look* like one class early (e.g., "meeting minutes" → memo) but belong to another (a memo is not minutes; a budget recap is not a meeting note). The model latches onto the trigger word, commits at check ≤7, and never reaches the checks that would disambiguate.
