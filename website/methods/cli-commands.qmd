...
subtitle: 'Every useful terminal command: setup, data pipeline, EDA, cost estimation, Braintrust evaluation.'
toc: true
toc-depth: 3
---

::: {.callout-tip title="Quick Start"}
Run all commands from the **repository root**. Each script supports `--help` for its full
option list (e.g. `python scripts/braintrust/braintrust_report.py --help`).
:::
...
::: {.callout-note title="Dataset Names"}
...
# 3. Resize/sample to fixed square sets (2550×3300, 1024×1024, ...) — edit configs in the script
...
::: {.callout-note title="RESEARCH_FUNDING_API_KEY"}
...
::: {.callout-warning title="Never Commit API Keys"}
