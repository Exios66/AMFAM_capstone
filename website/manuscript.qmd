...
This work asks whether a **zero-shot, prompt-engineered** alternative built from **general-purpose vision-language models (VLMs)** can match that performance envelope at a fraction of the engineering and cost overhead. Rather than fine-tune a model, we iteratively refine a structured classification prompt and evaluate each version against balanced slices of RVL-CDIP through Braintrust, with OpenRouter as the model gateway [@openrouter;@braintrust]. **No training is performed; every accuracy gain is attributable to prompt design.** The premise is not idle: prior work shows that zero-shot prompting of general-purpose LLMs can match or exceed expert and crowd-worker annotation accuracy on classification tasks [@tornberg2023zeroannotation], and that LLM outputs are acutely sensitive to prompt wording, framing, and instruction structure [@bisbee2024syntheticreplacements;@qu2024publicopinion]. We scale that result from text annotation to document-image classification, and we measure the variance that prompt engineering induces rather than assuming it away.

::: {.callout-important title="Key Finding: Prompt Engineering as the Dominant Lever"}
Prompt engineering alone moves **exact-match accuracy** on a fixed 160-image slice from **80.1% (v1)** to **99.4% (v11.8)** — a **+19.3 *pp* gain** with the same model and the same images. The paired bootstrap confirms this is not slice over-fitting: on 898 shared images, v11.8 outperforms the v0 baseline by **+13.0 *pp*** (0.732 → 0.862, 95% *CI* [0.101, 0.159], *p* < .001).
:::
...
**Thread 6 — Prompt engineering as the dominant lever on performance variance.** A parallel literature, built around LLM annotation and simulation rather than document classification, establishes that prompt design — not just model choice — is a first-order determinant of LLM performance. Törnberg showed that zero-shot ChatGPT-4 outperforms both crowd workers and expert coders on political-text annotation, demonstrating that a well-specified zero-shot prompt can beat human baselines [@tornberg2023zeroannotation]. Argyle et al. demonstrated that persona-specifying prompts let LLMs stand in for human survey samples, with output fidelity governed by prompt framing [@argyle2023outofonemany]; Chuang et al. showed role-playing alignment improves when agents are seeded with beliefs drawn from empirically estimated human belief networks [@chuang2024beyonddemographics]; and Gudiño et al. built LLM agents that augment citizen-preference data in an augmented-democracy setting [@gudino2024augmenteddemocracy]. The cautionary counterpart is equally well documented: Bisbee et al. found that "the effects and importance of prompt engineering affect every LLM output," with minor prompt rewording measurably shifting response distributions and results drifting across a three-month window [@bisbee2024syntheticreplacements]; Qu and Wang documented performance and bias variation across prompt configurations in public-opinion simulation [@qu2024publicopinion]; Kaiser et al. [@kaiser2025simulatingopinions], Kim and Lee [@kim2023aiaugmentedsurveys], and Lee et al. [@lee2025syntheticdataopinion] all report that prompt and framing choices condition the fidelity of LLM-generated survey data; and Anthis et al. frame LLM social simulation as a promising but validation-sensitive research method [@anthis2025llmsimulation]. Our work operationalizes this finding in the document domain: where those studies treat prompt sensitivity as a threat to external validity, we treat it as the experimental lever, versioning prompts append-only and measuring each version's effect on exact-match accuracy.
...
We record two cost measures per run:

- **Expected cost** = (prompt tokens × input price + completion tokens × output price) / 1e6 — a list-price projection from measured token counts
- **Actual cost** = the sum of OpenRouter's billed per-row `usage.cost`

Per-model single-image metrics are captured by running one image per model and recording the actual API response (tokens and `usage.cost`); input/output per-million-token prices are then **derived from those measured metrics**, so defaults reflect observed behavior rather than list prices.

## Monte Carlo analyses {#sec-montecarlo}

All simulations follow the same pattern: draw bootstrap samples from the committed per-image corpus (`reports/monte_carlo/corpus.jsonl`, 1,912 rows), compute the statistic on each draw, and report the mean and 95% interval [@efron1993bootstrap]. We analyze:

- **(a) Ensemble voting** — committees of *K* independent reads with majority vote [@condorcet1785]
- **(b) Confidence-gated escalation** — routing the lowest-confidence α fraction to a stronger model at 3× cost
- **(c) Hasty-stop triggers** — accumulated local effects (ALE) of reasoning-trace features on correctness
- **(d) Failure pipeline** — a Markov model of retry/fallback behavior at scale
- **(e) Few-shot exemplar mining** — injecting exemplars for the top confusion pairs
...
::: {.callout-warning title="Generalization Caveat"}
Accuracy at v11.8 attenuates with slice size: **99.4% at 160** → **87.2% at 320** → **89.1% at 480** → **83.1% at 800** → **82.6% at 1,120**. The falloff reflects larger slices exposing more intra-class layout variation and noisier scans, not any prompt-specific failure. The 160-image curated slice is cleaner than the full benchmark.
:::
...
::: {.callout-note title="Prompt Caching Drives the Cost Advantage"}
Measured steady-state cost for the primary model is **≈ $0.0004 per image**, and the 1,120-image run billed **$0.4937** versus an expected list-price projection of **$0.6815** — actual cost runs **27.5% below** list projection across runs. ~63% of prompt tokens were cache hits billed at ~10% of input price.
:::
...
::: {.callout-warning title="Measured Routing Negative Result"}
The measured escalation slice **regressed** from 0.667 to 0.625 on 48 low-confidence images — the same model at higher reasoning effort did not help. Escalation requires a genuinely stronger model, not more reasoning effort.
:::
...
::: {.callout-warning title="Measured Exemplar Negative Result"}
The v18 exemplar appendix **did not improve accuracy** (−4.2 *pp* on the 48-image exemplar slice). Adding more worked-example text does not fix the runner-up-vs-final decision; the fix requires a decision-rule change, not more exemplar verbosity.
:::
...
- **Prompt drift.** Bisbee et al. document that identical prompts yield different outputs over time, and Qu and Wang show prompt-conditioned performance variation [@bisbee2024syntheticreplacements;@qu2024publicopinion]. Our accuracy figures are point-in-time measurements at fixed model versions via OpenRouter; a provider-side model update could shift them.
